...
class DataConversionAndPlotting():
    ...
    def plot(self):
        ...
        for i in sorted(index_of_group):
            indexing.append(self.data.iloc[temp,0])
    ...
    def Survival(self):
        ...
        for i in range(len(self.data.index)):
            count=0
            measure=list(self.data.iloc[i,1:])[::-1]
            for j in measure:
                if np.isnan(j):
                    count+=1
                else:
            ...
            measure[:count] = [0 for i in range(len(measure[:count]))] 
            measure[count:] = [1 for i in range(len(measure[count:]))]
            survivalData=measure[::-1]
            #print(list(self.data.iloc[i,1:]))
            #print(survivalData)

            #print(survivalData)
            new_df=pd.concat([new_df,pd.DataFrame(data=survivalData)],axis=1)
            
        new_df=new_df.T
        new_df.index=self.data['Days after inoculation']
        new_df.to_excel("surivalstatus.xlsx")
